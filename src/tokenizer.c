...
#define TABSIZE 8
#define Py_FatalError(s) fprintf(stderr, "%s\n", s)
#define Py_CHARMASK(c)		((unsigned char)((c) & 0xff))
#define Py_ISUPPER(c) ((c) >= 'A' && (c) <= 'Z')
#define Py_ISLOWER(c) ((c) >= 'a' && (c) <= 'z')
#define Py_ISNUM(c) ((c) >= '0' && (c) <= '9')
#define Py_ISALPHA(c) (Py_ISUPPER(c) || Py_ISLOWER(c))
#define Py_ISALNUM(c) (Py_ISALPHA(c) || Py_ISNUM(c))
...
//
//static char *
//get_coding_spec(const char *s, ssize_t size)
//{
//    ssize_t i;
...
//
//static int
//check_coding_spec(const char* line, ssize_t size, struct tok_state *tok,
...
tok_nextc(register struct tok_state *tok)
{
    for (;;) {
        ...
        }/* prompt is NULL because we only use file as input
        if (tok->prompt != NULL) {
...
tok_get(register struct tok_state *tok, char **p_start, char **p_end)
{
 ...
 again:
  ...
  letter_quote:
    /* String */
    if (c == '\'' || c == '"') {
        ssize_t quote2 = tok->cur - tok->start + 1;
        int quote = c;
        int triple = 0;
        int tripcount = 0;
        for (;;) {
